...
module.exports = {
  ...
  createNewIngredient: (req, res) => {
    IngredientModel.create(req.body)
      .then((newIngredient) =>
        IngredientModel.findOne({ _id: newIngredient._id })
      )
      .then((newIngredientPopulated) =>
        res.status(201).json(newIngredientPopulated)
      )
      .catch((err) => {
    ...
    IngredientModel.deleteOne({ _id: req.params.id })
      .then((result) => {
        if (result.deletedCount === 0) {
          ...
          res
            .status(200)
            .json({ messages: "Ingredient deleted", result: result });
        }
      })
      .catch((err) =>
        res.status(500).json({ message: "Something went wrong", error: err })
